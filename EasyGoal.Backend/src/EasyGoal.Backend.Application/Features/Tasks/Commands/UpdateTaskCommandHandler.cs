...
public sealed class UpdateTaskCommandHandler : IRequestHandler<UpdateTaskCommand>
{
    ...
    public UpdateTaskCommandHandler(IRepository<Task> taskRepository, IRepository<Goal> goalRepository, ICurrentApplicationUser currentApplicationUser)
    {
    ...
    public async System.Threading.Tasks.Task Handle(UpdateTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        task.Update(request.Name, request.StartTime, request.EndTime, request.Notes);
